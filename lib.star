...
def generate_build_number():
    ...
    build_number_raw = get_metadata("KHULNASOFT_BUILD_NUMBER") or "0"

    # Calculate current build number
    build_number = int(build_number_raw) + 1

    # Set new build number
    return set_metadata("KHULNASOFT_BUILD_NUMBER", str(build_number))


def get_metadata(key, repository_id=None):
    repository_id = repository_id or env.get("KHULNASOFT_REPO_ID")
    variables = {
...
def execute(query, variables=None):
    ...
    headers = {}
    if "KHULNASOFT_TOKEN" in env:
        headers["Authorization"] = "Bearer " + env["KHULNASOFT_TOKEN"]

    response = http.post(
        url=env.get("KHULNASOFT_API_HOST", "https://api-ci.khulnasoft.com/") + "graphql",
        headers=headers,
        json_body=body
    )
